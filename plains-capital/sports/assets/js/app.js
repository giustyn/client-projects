$(document).ready(function () {    
    ...
    var feed = url.getSearchParam("feed") || "sports", // news, sports, celeb, fin, weather, markets
    ...
    function setWeather(weather) {
        ...
        $("#forecast .day").each(function (i, el) {
            ...
            var iconURL = iconPath + forecast[i].Icon.split('.').slice(0, -1).join('.') + ".svg";
            var $el = $(el);
        ...
        $("#current-icon").attr("src", iconPath + curweather.Icon.split('.').slice(0, -1).join('.') + ".svg");
    ...
    function loadFeeds() {
        ...
        getArticles(local, getIndexes($articleLimit))
            .done(function (response) {
                if (response.articles) {
                    // console.log("Loading local data..");
                    console.log("source: " + local, '\n', response.articles);
                    // console.log(response.articles)
                    getWeather(localBasePath + "/weather/", "weather.json")
                        .done(function (response) {
                            setWeather(response);
                        });
                    $bumper.attr("src", "./assets/" + videoSource);                    
                    ...
                    return getArticles(server, getIndexes($articleLimit))
                        .done(function (response) {
                            // console.log("Local data not found. Fetching server data..");
                            console.log("source: " + server, '\n', response.articles);
                            // console.log(response.articles);
                            getWeather(serverFeeds['weather'], zipcode + ".json")
                                .done(function (response) {
                                    setWeather(response);
                                });
                            $bumper.attr("src", serverBasePath + "/navori/assets/" + feed + videoSource);                            
